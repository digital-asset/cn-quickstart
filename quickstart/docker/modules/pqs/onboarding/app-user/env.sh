...
if [ "$AUTH_MODE" == "oauth2" ]; then
  ...
  create_user "$APP_USER_PARTICIPANT_ADMIN_TOKEN" $AUTH_APP_USER_PQS_USER_NAME $AUTH_APP_USER_PQS_USER_NAME "" "canton:2${PARTICIPANT_JSON_API_PORT}"
  grant_rights "$APP_USER_PARTICIPANT_ADMIN_TOKEN" $AUTH_APP_USER_PQS_USER_NAME $APP_USER_PARTY "ReadAs" "canton:2${PARTICIPANT_JSON_API_PORT}"
